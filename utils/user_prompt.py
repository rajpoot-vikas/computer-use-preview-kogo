...
1.  **Navigate to Search Portal (If Necessary):**
   ...
   - **Actions:**
      - Scan the page for navigation elements like menus, buttons, or links with keywords such as "Search," "Property," "Tax," "Parcel,", "MAP Number", "Tax Map", "Assessments" or similar terminology ."
...
3.  **Configure Search Criteria:**
   ...
   - **Actions:**
    ...
    - **Parcel Input:** Locate the input field(s) for the parcel number (e.g., "Parcel Number," "Account Number," "Property ID," "MAP NUMBER"). click exactly one the input box [____], locate properly. 
        ...
        - **Multiple Fields:** If the input is split into several boxes, split `{parcel_number}` by its delimiter (e.g., '-', ' ') and enter each part into the corresponding box.
...
5.  **Process Search Outcome:**
   ...
   - **Scenario C: No Results:** If no results are found, the task is complete with a "not found" status. Mark Task as complete no need to do anything.
...
7.  **Confirm and Finalize:**
   ...
   - **Actions:** Wait for the tax/payment page to load. Once a page containing terms like "Amount Due", "Total Tax", "Taxes Payable", or similar financial details for the correct parcel is visible, 

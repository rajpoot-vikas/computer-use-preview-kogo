...
USER_FLEXIBLE_SPLIT_PARCEL_PROMPT = """
**Objective:**
...
3.  **Configure Search Criteria:**
   ...
   - **Actions:**
    ...
    - **Search Type:** If a choice is offered (e.g., "By Parcel," "By Owner"), select the option for "Parcel Number," "Account ID," or a similar identifier.
    - **Year Selection:** If a tax year selector is present, set it to `{search_year}`.
    - **Parcel Input:** Locate the input field(s) for the parcel number (e.g., "Parcel Number," "Account Number," "Property ID," "MAP NUMBER").
        - **Single Field:** If it's a single input field, enter the full `{parcel_number}`.
        - **Multiple Fields:** If the input is split into several boxes, split `{parcel_number}` by its delimiter (e.g., '-', ' ') and enter each part into the corresponding box. For example, if the parcel is "12-345-678" and there are three boxes, enter "12" in the first, "345" in the second, and "678" in the third.
    - **Self-Correction:** If any selection (like year or search type) clears other fields, re-enter the necessary information. Ensure all required fields are correctly populated before proceeding.
